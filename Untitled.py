...
train_dataset = YUVImageDataset(yuv_dir, diff_map_dir, image_size=(720, 1280), 
                                mask_threshold=0.1, mode='train', triplets_list=train_triplets)

test_dataset = YUVImageDataset( yuv_dir, diff_map_dir, image_size=(720, 1280), 
                               mask_threshold=0.1, mode='test', triplets_list=test_triplets)
...
def visualize_results(generator, test_loader, num_images=5):
    ...
    with torch.no_grad():
        for i, (masked_y, y_channel, mask, _) in enumerate(test_loader):
            ...
            for j in range(min(len(generated), num_images)):
                fig, axs = plt.subplots(1, 3, figsize=(18, 6))
                
                # Masked Input Image
                axs[0].imshow(masked_y[j], cmap='gray')
                axs[0].set_title("Masked Input Image")
                axs[0].axis('off')
                
                # Ground Truth Image
                axs[1].imshow(y_channel[j], cmap='gray')
                axs[1].set_title("Ground Truth Image")
                axs[1].axis('off')
                
                # Generated Image
                axs[2].imshow(generated[j], cmap='gray')
                axs[2].set_title("Generated Image")
                axs[2].axis('off')
                
                plt.show()
                
            if i * len(generated) >= num_images:
                break
    generator.train()
